...
def transformData ( training, validation, test ):
    #csv -> dataframe
    #names per le colonne
    #vedi come funziona error_bad_lines=False
    ...
    #facciamo uscire 3 oggetti - utilizziamo la funzione getXY:
    #1) vettore delle X, Y e L
    x_train, y_train, l_train = getXY(dfTrain)
    x_val, y_val, l_val = getXY(dfValidation)
    x_test, y_test, l_test = getXY(dfTest)

    #scaling del dataset
    #dichiariamo uno scaler MaxAbsScale
    #Gli scaler li troviamo nel modulo sklearn
    scaler = MaxAbsScaler()

    #normalizzazione x_train: fit determina il massimo, trasfornm scala
    x_train = scaler.fit_transform(x_train)
    #il fit dello scaler si fa solo la prima volta
...
def getXY ( inDataframe ):
    #1.
    #   da inDataframe ci prendiamo solo le colonne x
    #   con .values trasformiamo in un array numpy
    ...
    #   impacchettiamo tutto in una unica colonna
    Y=np.column_stack((bitA, bitB))

    #3.
    #   label originali: stringhe
